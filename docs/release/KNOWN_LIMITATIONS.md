...
- The agreed first-Beta runtime target is Windows 10 x64, Windows 11 x64, and Windows 11 ARM64. Final Windows 11 x64 acceptance remains open on the available host; Windows 10 x64 and Windows 11 ARM64 require matching environments. x86 is build/inspection coverage only and is not a first-Beta support claim. Cross-architecture compilation is not device evidence, and untested Windows 10 editions such as LTSC 2019 must not be advertised as supported.
