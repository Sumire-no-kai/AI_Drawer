...
Completed the current-host no-account acceptance after the implementation review. Core passed 47 checks; the application harness passed 23 non-UI and 27 complete UI Automation checks; x64 Debug and Release production builds completed with zero warnings and errors. The isolated runtime harness passed 20 fast checks and 21 checks with a real five-minute retained-page interval, including five pages, same-provider creation, rename, physical drag ordering, search, shortcuts, release/recreation, recovery, tray, exact Exit, and zero isolated-profile residue.

The run found and fixed four user-visible defects: the default 150%-scale window could not show split view, a selected fifth tab stayed outside the hidden-overflow strip, the native drag implementation lost movement inside the horizontal scroller, and provider security warnings permanently consumed page space. Split view now works at the default size and explains narrow-window suspension, focused tabs scroll into view, pointer drag ordering is persisted, and warnings can be dismissed after review. Prompt actions also expose their dynamic labels to UI Automation. ChatGPT guest conversation continuity passed one isolated visual run; Gemini accepted a guest submission but then requested sign-in; Claude exposed only sign-in. No account action was taken. See `docs/testing/HOST_ACCEPTANCE_2026-09-21.md` for evidence and remaining Gates.
...
- Connected the approved fixed Microsoft Forms destination, `https://forms.cloud.microsoft/r/WLQySVad7g`, to the native Feedback & Support surface and to a small Home action reading **Can't find the AI you want? Tell me.** Both actions use the system browser and append no application, build, provider, account, or page data. Provider-evidence and private-security routes remain separate.
- Added an isolated Debug runtime-acceptance harness. It creates a generated directory under the Windows temporary root, uses only the exact `https://example.com/` origin, suppresses real authentication origins, preconfigures native onboarding/support state, and exercises real WebView2 lifecycle and profile APIs without provider login, prompts, purchases, or normal application data. Debug profile-action activation accepts only three fixed actions, a 32-byte marker, and a non-reparse generated acceptance root under the system temporary directory; Release contains no action hook or provider-origin override.
- Added a browser-process watcher alongside WebView2's environment event. Browser-exit signals are deduplicated, ignored during intentional profile cleanup, detached during environment replacement and shutdown, and recreate the environment with the same isolated profile. The acceptance probe exposed the previous browser-tree recovery failure before this implementation; the repeated fast run then recovered a new browser process and successful navigation.
- Hardened the application test host so it never auto-initializes as an MSIX and fails before any test if the Debug-only isolated data root is not active. Failure output now reports the innermost exception type and stack. Added focused feedback, browser-recovery, test-hook, download non-execution, and directory-collision checks. Corrected the Core harness to print its measured check count instead of a stale literal.
- Added internal-only MSIX version override support by using the reviewed MakeAppx tool from the restored Windows SDK package to rebuild an unsigned candidate in a generated temporary directory. The source package manifest is not edited, the temporary path is validated before recursive cleanup, and the resulting candidate is re-inspected before its checksum and metadata are emitted.
- Corrected the recorded licenses for Microsoft.WindowsAppSDK and Microsoft.Windows.SDK.BuildTools to the Microsoft Software License Terms.

### Verification and failure record

- Production x64, x86, and ARM64 Debug and Release builds passed with warnings as errors. Compatibility Lab and application-test x64 Debug and Release builds also passed with warnings as errors. All four maintained projects passed `dotnet format --verify-no-changes`; both PowerShell tools passed parser checks. Core passed 47 policy checks and the isolated Debug application harness passed 21 non-GUI checks.
- A real five-minute no-account run passed cold start (436 ms), single-instance redirection, four-workspace hard-limit pressure, Keep active persistence, five-minute protection expiry, steady-state release, WebView2 process/memory capture, disposed-workspace restoration, renderer recovery, and GPU recovery. Its report stopped at the browser-tree defect found during that run. After the fix, a fast run additionally passed real Clear cache, Reset provider, Reset all, browser-environment recreation, close-to-tray, global-shortcut restoration, and second-launch restoration. The final exact Exit automation ID and post-exit residue steps have not been rerun because the desktop GUI approval service disconnected; source and non-GUI checks do not substitute for that final runtime evidence.
- Clean commit `c851525` produced inspected 1.0.0.0 x64 and ARM64 candidates with SHA-256 `9c1eace04ce5edcf2a0de40761c46f17bfd428ebf2df91ec0fc404fbcb2341f7` and `d64f97d883be91c07e6ee3f4cd8bc7056e040e586b8e31cfe880b7d6f49796c0`. An earlier dirty branch probe also rebuilt and inspected an internal 1.0.0.1 x64 candidate, proving the version-override path before package registration was attempted. Each candidate contains only the reviewed `runFullTrust` capability, the disabled startup task, the requested architecture and version, no signature, and no private-key material. The toolchain still lacks `mspdbcmf.exe`, so no symbols package was generated.
- Exact-package installation did not pass. Windows deployment rejected the main package with `0x80073CFB`, reporting a conflicting unpackaged `AIDrawer.App` registration that is not visible to the current-user package query; the host policy also denied the all-users query needed to identify it. The update was therefore rejected before registration, uninstall could not be meaningfully exercised, and no AI Drawer package remains visible to the current user. This is a host-state blocker, not installation evidence.
- Codex Security Standard scan `8745c8a7-9be7-42d9-858d-5f77e4a51055` completed against immutable master revision `0c7fd79593925bec364b67779102826bac86b0eb`. It found no backdoor, covert telemetry, sensitive page-to-native collection, arbitrary command execution, automatic download execution, hardcoded secret, or unexpected capability. Two Low findings remain open: non-Gemini purchase-route lists still lack exact observed rules, and Grok still allows the full exact `x.com` origin. Both require privacy-safe account/runtime evidence rather than guessed routes.

### Remaining evidence boundary

- Rerun the final fast and five-minute no-account GUI acceptance on the reviewed branch to close exact Exit, tray/shortcut/WebView2 residue, final steady-state metrics, and profile-directory cleanup evidence.
- Resolve the hidden Windows development registration on a controlled package-test user or clean machine, then repeat exact 1.0.0.0 install, 1.0.0.1 update, launch-on-startup registration, uninstall, rollback, and data-retention decisions. Do not remove an unidentified all-users registration from this machine.
- Account/provider-runtime, Windows 10, ARM64-device, Narrator, 200% DPI/text scaling, signed Store identity, signing/provenance, and Microsoft Forms ownership/notification/retention/monitoring remain separate release Gates.
