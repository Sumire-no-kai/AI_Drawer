...
Conversation tabs now retain their opened pages across switching and hiding. Names, order, focus, and an optional two-pane layout are independent of live WebViews. Same-provider tabs share a sign-in profile. Pages are created lazily and released only through explicit user actions or recovery; memory grows with opened pages. See [ADR 0003](docs/adr/0003-retained-conversation-tabs-and-split-view.md).

M3 implementation now applies a fail-closed, exact-origin navigation boundary while currently unavailable M2 tests remain explicitly deferred rather than passed. Main workspaces, their frames, and controlled provider popups embed only reviewed HTTPS provider or authentication origins; certificate-error navigation is explicitly cancelled. Unreviewed top-level external links require native confirmation before a handoff retaining only reviewed reference parameters for explicit user links to the system browser, while external frames remain blocked. Known purchase paths are cancelled and show a native explanation only: AI Drawer never opens or processes provider purchases. This does not change any provider compatibility status or establish that provider-specific authentication, external-link, billing, checkout, or payment flows have passed runtime validation.

M4 quality work now includes a pinned .NET SDK, clean-checkout CI with x64/x86/ARM64 production builds, privacy-safe issue templates, GitHub Private Vulnerability Reporting, release-gate documentation, and a manually triggered internal MSIX candidate workflow. The workflow can generate separately validated x64, x86, or ARM64 candidates. Each candidate is deliberately unsigned, carries a SHA-256 checksum and machine-readable source/package metadata, and is explicitly marked as ineligible for public release. Its validation checks the package architecture, disabled startup-task declaration, reviewed `runFullTrust` capability set, absence of a signature/private-key file, and checksum metadata. It validates the packaging pipeline only: it is not an installable Public Beta, and it does not resolve signing, public identity/versioning, the independent website, or the deferred M2/M3 runtime Gates.
...
Historical baseline before the retained-tab redesign: production x64, x86, and ARM64 Debug/Release builds pass with `Recommended` analyzers and warnings treated as errors. The framework-free Core harness passes 42 locator, lifecycle, navigation-support, shortcut, download-name/collision, window-placement, WebView-security, recovery, and settings-normalization checks. The latest application harness baseline passes 17 non-GUI checks and all 21 checks with four isolated UI flows; the recovery-browser and Feedback & Support additions extend that coverage and are re-verified on their implementation branch before merge. These runs cover Welcome/restart recovery, corrupt-session backup, settings, support-reminder persistence, close-to-exit behavior, provider-catalog policy, and structural home-screen interaction boundaries using generated temporary data roots; they do not register a package, use a real provider account, submit a prompt, visit payment flows, open external browser destinations, or access normal-browser data. A clean-source x64 internal package probe also passed MakeAppx validation and confirmed the packaged startup-task declaration, reviewed capability set, unsigned identity, private-key absence, and SHA-256 output. This is still not M2/M3 acceptance or a provider/performance claim: real account-conversation restart, live profile reset/cache/download behavior, packaged startup registration, renderer-specific recovery, popup/provider URL behavior, accessibility breadth, x86/ARM64 device execution, and the required Windows 10/11 matrix remain open. There is no public end-user build yet.
...
- lazy WebView creation with retained background pages and explicit release;
...
- settings for shortcut, startup, tray, window, workspace restoration, and provider-profile data controls;
...
## Conversation tabs and page retention

Opened pages remain alive when switching tabs or hiding the window. Memory use grows with the pages you open. Closing a tab or explicitly releasing its page can lose drafts, uploads, voice, or ongoing generation; reopening an address cannot reconstruct this transient state. Provider scripts and the operating system can still interrupt a page.

- Ctrl+T opens another tab with the current provider; Ctrl+N or the plus button opens the provider chooser.
- Ctrl+Tab / Ctrl+Shift+Tab cycle tabs; Ctrl+1–9 selects by tab position.
- Right-click a tab to rename, reorder, open another with the same provider, release its page, or show it beside the current tab. The find button searches open tab names.
- Split view shows two existing pages. Selection replaces the focused pane; ending split keeps the focused page. Narrow windows temporarily show one pane and retain the pair.
- Same-provider tabs share one sign-in profile. Isolated accounts remain outside the MVP.
- Restart restores tab metadata and supported encrypted conversation addresses, not page memory. Unsupported addresses return to provider home.
- Schema 1 is backed up before the first schema-2 write. Older builds reject the new schema; rollback requires the retained backup. User-entered names are saved locally as plaintext metadata.
- Corrupt or newer sessions remain write-blocked until Retry or explicit Back up and continue. Provider content never enters session storage.

The new lifecycle has compiled and received source review; local runtime tests and GUI acceptance are deferred. Earlier lifecycle acceptance applies only to the historical automatic-disposal implementation. No new provider compatibility or memory-capacity claim follows from this change.
...
The script creates an ignored directory under `artifacts/beta-candidates`, verifies that the main x64 MSIX has no signature block or private-key files, the expected disabled startup task, and only the reviewed `runFullTrust` capability; it then writes `SHA256SUMS.txt`, `candidate-manifest.json`, and the current known limitations. `-AllowDirtySource` is only for local development probes and is recorded in the candidate metadata. See the [Public Beta release checklist](docs/release/BETA_RELEASE_CHECKLIST.md), [Store-submission preparation](docs/release/STORE_SUBMISSION_PREPARATION.md), [versioning and rollback](docs/release/VERSIONING_AND_ROLLBACK.md), and the [release-notes template](docs/release/RELEASE_NOTES_TEMPLATE.md) before treating any artifact as releasable.
