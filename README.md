...
M3 development has started with a navigation-security foundation while currently unavailable M2 tests remain explicitly deferred rather than passed. Main workspaces and controlled provider popups share one fail-closed top-level navigation classification, and certificate-error navigation is explicitly cancelled. This does not change any provider compatibility status or establish that provider-specific authentication, external-link, billing, checkout, or payment flows have passed runtime validation.
