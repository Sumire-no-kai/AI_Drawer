...
M3 development has started with a navigation-security foundation while currently unavailable M2 tests remain explicitly deferred rather than passed. Main workspaces, their frames, and controlled provider popups share one fail-closed navigation classification that distinguishes reviewed provider application and authentication origins from external, purchase, and unsupported navigation. Certificate-error navigation is explicitly cancelled. This does not change any provider compatibility status or establish that provider-specific authentication, external-link, billing, checkout, or payment flows have passed runtime validation.
